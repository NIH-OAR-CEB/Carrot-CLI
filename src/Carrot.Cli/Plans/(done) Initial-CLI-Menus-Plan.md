...
- Add an internal help-topic catalog, embedded-resource content provider, and lightweight Markdown renderer. Store canonical runtime content as one file per topic under `docs/help/` and embed those files in `carrot-cli.dll`, eliminating working-directory and deployment-file dependencies.
