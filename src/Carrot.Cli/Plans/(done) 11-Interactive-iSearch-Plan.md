...
- Use `/health` to check availability, `/datasets` for selectable database names, and the dataset-scoped `GET /search/{dataset}` route for a bounded free-text query.
...
- **Queries:** Use the dataset-scoped `GET /search/{dataset}` route with URI-encoded free-text/Lucene input because the live body-based `POST /search` route currently returns HTTP 500 even for `*:*`. The first phase accepts an unqualified free-text/Lucene query only; it does not promise field-qualified syntax because fields have not yet been discovered with `GET /fields/{dataset}`. Request no more than 100 records, set `defaultOp` to `AND`, and render `returnedCount`, `totalCount`, and the returned `results` in a terminal-height-aware view. A later phase can add field discovery and paging from the response cursor.
...
- A submitted nonempty query is sent as bounded, URI-encoded `GET /search/{dataset}` parameters for the selected database with `defaultOp=AND`, and its returned counts and records are readable without writing files.
