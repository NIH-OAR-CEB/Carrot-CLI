...
Named `preview` uses the atomic JSON writer for one complete indented UTF-8 request artifact after `/list` validation and never calls `/cluster`. Existing destinations require explicit overwrite permission; failure or cancellation preserves the prior file and removes temporary output. Named process response-artifact naming remains deferred, so interactive Excel export still creates no JSON sidecars.
