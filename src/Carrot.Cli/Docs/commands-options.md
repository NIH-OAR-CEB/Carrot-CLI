...
Interactive Preview Request additionally offers **Save Workbench JSON**. It writes only the root JSON array of document records, which can be uploaded to Carrot Workbench's **Local file** source. This intentionally differs from the named preview artifact, which is the complete `/cluster` request wrapper.
