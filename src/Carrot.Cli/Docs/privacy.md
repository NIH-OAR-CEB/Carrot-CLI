...
- **Categorize iSearch Results** sends the retained `nihApplId`, `title`, `abstract`, and `specificAims` values to the selected Carrot endpoint. It does not fetch more iSearch data. Category results remain in memory until the categorized view is left or the application exits; **Save Categorized iSearch Results to Excel** is explicit, local, atomic, and creates no JSON sidecar or log.
