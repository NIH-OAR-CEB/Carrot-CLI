...
Successful processing opens five-document result pages automatically. Each row shows its submitted Carrot index, source/title, assigned or unassigned status, membership count, category paths, and unrounded scores. Memberships can overlap and can be nested. Empty cluster results are valid and show every document as unassigned. Every result page offers **Save Processed Results to Excel**. Next Page is the default whenever available; Save is the default on the final page, and Previous Page plus Escape/Back are supported. Saving returns to the same result page. A success also adds **View Processed Results** and **Save Processed Results to Excel** to Batch Actions. A later failure leaves that previous success and the prepared batch available.
