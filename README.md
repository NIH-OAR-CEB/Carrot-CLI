...
To enable **iSearch**, store `iSearch:apiKey` and `iSearch:contactEmail` in User Secrets. The interactive route checks `/health`, discovers databases live, lets you select a configured return dataset from `iSearchReturnTypes.Results`, sends its record fields as `fl`, and offers **Build Advanced Query** for live-field selection, `qf`/`fq` filters, and updated-date bounds. Advanced requests are shown as pretty JSON for edit or confirmation before submission. The prompt-free `isearch` command exposes the same `q`, repeated qf/fq, `defaultOp`, `rows`, and update-date controls as `--query`, `--query-field`, `--filter-query`, `--default-op`, `--rows`, `--updated-after`, and `--updated-before`; configured return fields remain selected by `--result-dataset`. The named command uses `--max-results` or explicit `--all-results`, never prompts, and has no sort option. Results report shared total/current/result-page cardinality with the response cursor, distinguish local **Next Display Page** navigation from one-request **Fetch Next Result Page** retrieval, and offer **Fetch All Pages** below it to sequentially walk every remaining page while the Search Summary shows actual loaded-record progress. **Save iSearch Results to Excel** includes all pages walked in the current query session. **Categorize iSearch Results** sends the loaded `nihApplId`, `title`, `abstract`, and `specificAims` fields through the shared Carrot path, followed by an explicit categorized Excel export. The key is sent only as a cookie, requests are limited to 100 records per page, and continuation respects the documented authenticated one-second interval. See [iSearch help](src/Carrot.Cli/Docs/isearch.md).
...
| `isearch` | Runs a prompt-free iSearch search with basic or advanced query options and optional Excel/Carrot output. |
