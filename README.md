...
To enable **iSearch**, store `iSearch:apiKey` and `iSearch:contactEmail` in User Secrets. The interactive route checks `/health`, discovers databases live, lets you select a configured return dataset from `iSearchReturnTypes.Results`, sends its record fields as `fl`, reports shared total/current/result-page cardinality with the response cursor, distinguishes local **Next Display Page** navigation from one-request **Fetch Next Result Page** data retrieval, and offers **Save iSearch Results to Excel** for all pages walked in the current query session. The key is sent only as a cookie and requests are limited to 100 records per page. See [iSearch help](src/Carrot.Cli/Docs/isearch.md).
