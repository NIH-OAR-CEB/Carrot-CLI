...
The persistence boundary for future named `preview` and `process` commands can write complete indented UTF-8 request and response JSON artifacts through the same atomic temporary-sibling pattern. Existing JSON destinations require explicit overwrite permission, and failure or cancellation preserves the prior destination. Named-command filename selection and orchestration remain deferred, so the current interactive workflows do not create these sidecars.
