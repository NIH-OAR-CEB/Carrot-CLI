...
Batch Actions provide **View Prepared Results**, **Preview JSON Package**, **Process Prepared Items**, **Start Over**, **Help**, and **Back to Main Menu**. JSON preview displays the exact indented request body with the configured default language and algorithm and every ready document's complete title/content. It excludes local source metadata, contacts no server, and writes no file. Processing reports the number of ready documents but remains pending; it does not contact Carrot or create Excel, JSON, or log files. Start Over and Back require confirmation before discarding extracted content.
