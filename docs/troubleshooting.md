...
- Scheduled task starts but produces no output: set the `Start in` directory, redirect standard output/error to the task runner's configured destination, and inspect the task's last result code. Named `server-info` returns `0` on success, `1` for invalid configuration, `4` for endpoint or `/list` failure, and `130` for cancellation. Interactive Server Information reports safe diagnostics and returns to its submenu.
...
The interactive shell, document preparation, interactive Server Information, paged and visually wrapped local JSON package preview, Process Prepared Items API integration, correlated result paging, explicit atomic Excel export, atomic JSON persistence boundary, Markdown help, About output, generated command metadata, and named Server Information command are implemented. Preview JSON Package displays the complete request locally without contacting Carrot. Process Prepared Items sends that exact request and keeps request/response/results in memory; only the separate explicit Excel action currently writes a file. Preview Request, named JSON artifact orchestration, and named process orchestration remain deferred; a `NotImplementedException` with `Layout stub only.` from one of those named operational routes identifies a later seam.
